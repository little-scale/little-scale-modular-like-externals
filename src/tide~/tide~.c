...
typedef struct _tide
{
    ...
    long mode;                      // 0=AD, 1=Loop, 2=AR (default 1)
    long sync_enabled;              // 0=off, 1=on phase sync (default 0)
    long quantize_enabled;          // 0=off, 1=on parameter quantize (default 0)
    double slew_rate;               // 0.0-1.0 parameter slew limiting (default 0.0)
...
t_max_err tide_attr_setfreqscale(t_tide* x, void *attr, long argc, t_atom *argv);
t_max_err tide_attr_setmode(t_tide* x, void *attr, long argc, t_atom *argv);
t_max_err tide_attr_setsync(t_tide* x, void *attr, long argc, t_atom *argv);
t_max_err tide_attr_setquantize(t_tide* x, void *attr, long argc, t_atom *argv);
t_max_err tide_attr_setslewrate(t_tide* x, void *attr, long argc, t_atom *argv);
...
void ext_main(void* r)
{
    ...
    CLASS_ATTR_BASIC(c, "freqscale", 0);
    CLASS_ATTR_LABEL(c, "freqscale", 0, "Frequency Scale");
    CLASS_ATTR_ACCESSORS(c, "freqscale", 0, tide_attr_setfreqscale);
    CLASS_ATTR_ORDER(c, "freqscale", 0, "1");
    CLASS_ATTR_FILTER_CLIP(c, "freqscale", 0.0001, 1.0);
    CLASS_ATTR_DEFAULT_SAVE(c, "freqscale", 0, "1.0");
    CLASS_ATTR_STYLE(c, "freqscale", 0, "text");
    
    CLASS_ATTR_LONG(c, "mode", 0, t_tide, mode);
    CLASS_ATTR_BASIC(c, "mode", 0);
    CLASS_ATTR_LABEL(c, "mode", 0, "Operating Mode");
    CLASS_ATTR_ACCESSORS(c, "mode", 0, tide_attr_setmode);
    CLASS_ATTR_ORDER(c, "mode", 0, "2");
    CLASS_ATTR_FILTER_CLIP(c, "mode", 0, 2);
    CLASS_ATTR_DEFAULT_SAVE(c, "mode", 0, "1");
    CLASS_ATTR_STYLE(c, "mode", 0, "enumindex");
    
    CLASS_ATTR_LONG(c, "sync", 0, t_tide, sync_enabled);
    CLASS_ATTR_BASIC(c, "sync", 0);
    CLASS_ATTR_LABEL(c, "sync", 0, "Phase Synchronization");
    CLASS_ATTR_ACCESSORS(c, "sync", 0, tide_attr_setsync);
    CLASS_ATTR_ORDER(c, "sync", 0, "3");
    CLASS_ATTR_FILTER_CLIP(c, "sync", 0, 1);
    CLASS_ATTR_DEFAULT_SAVE(c, "sync", 0, "0");
    CLASS_ATTR_STYLE(c, "sync", 0, "onoff");
    
    CLASS_ATTR_LONG(c, "quantize", 0, t_tide, quantize_enabled);
    CLASS_ATTR_BASIC(c, "quantize", 0);
    CLASS_ATTR_LABEL(c, "quantize", 0, "Parameter Quantization");
    CLASS_ATTR_ACCESSORS(c, "quantize", 0, tide_attr_setquantize);
    CLASS_ATTR_ORDER(c, "quantize", 0, "4");
    CLASS_ATTR_FILTER_CLIP(c, "quantize", 0, 1);
    CLASS_ATTR_DEFAULT_SAVE(c, "quantize", 0, "0");
    CLASS_ATTR_STYLE(c, "quantize", 0, "onoff");
    
    CLASS_ATTR_DOUBLE(c, "slew_rate", 0, t_tide, slew_rate);
    CLASS_ATTR_BASIC(c, "slew_rate", 0);
    CLASS_ATTR_LABEL(c, "slew_rate", 0, "Parameter Slew Rate");
    CLASS_ATTR_ACCESSORS(c, "slew_rate", 0, tide_attr_setslewrate);
    CLASS_ATTR_ORDER(c, "slew_rate", 0, "5");
    CLASS_ATTR_FILTER_CLIP(c, "slew_rate", 0.0, 1.0);
    CLASS_ATTR_DEFAULT_SAVE(c, "slew_rate", 0, "0.0");
    CLASS_ATTR_STYLE(c, "slew_rate", 0, "text");
...
void* tide_new(t_symbol* s, long argc, t_atom* argv)
{
    ...
    if (x) {
        ...
        x->mode = 1;                    // Loop mode by default
        x->sync_enabled = 0;            // No sync by default
        x->quantize_enabled = 0;        // No quantization by default
        x->slew_rate = 0.0;             // No slew limiting by default
...
void tide_bang(t_tide* x)
{
    ...
    if (inlet == 0) {  // Frequency inlet accepts bangs for phase reset
        ...
        // Phase reset - no console output needed
    }
}
...
void tide_perform64(t_tide* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam)
{
    ...
    for (long i = 0; i < sampleframes; i++) {
        ...
        // Apply parameter quantization if enabled
        if (x->quantize_enabled) {
            current_shape = floor(current_shape * 16.0) / 16.0;  // 16 steps
            current_slope = floor(current_slope * 16.0) / 16.0;  // 16 steps
            current_smooth = floor(current_smooth * 8.0) / 8.0;  // 8 steps
        }
        
        // Apply slew rate limiting if enabled
        if (x->slew_rate > 0.0) {
            static double prev_shape = 0.0, prev_slope = 0.5, prev_smooth = 0.0;
            double slew_factor = x->slew_rate * 0.1;  // Scale slew rate
            
            current_shape = prev_shape + slew_factor * (current_shape - prev_shape);
            current_slope = prev_slope + slew_factor * (current_slope - prev_slope);
            current_smooth = prev_smooth + slew_factor * (current_smooth - prev_smooth);
            
            prev_shape = current_shape;
            prev_slope = current_slope;
            prev_smooth = current_smooth;
        }
        ...
        // Handle phase reset from bang message or sync
        if (x->reset_phase || (x->sync_enabled && i == 0)) {
        ...
        // Call Tides render function with mode selection
        int ramp_mode = x->mode;  // 0=AD, 1=Loop, 2=AR
        tides_render(
            x->poly_slope_generator,
            ramp_mode,                  // ramp_mode from attribute
...
t_max_err tide_attr_setfreqscale(t_tide *x, void *attr, long argc, t_atom *argv) {
    if (argc && argv) {
        double scale = atom_getfloat(argv);
        x->freq_scale = CLAMP(scale, 0.0001, 1.0);
    }
    return 0;
}
...
t_max_err tide_attr_setmode(t_tide *x, void *attr, long argc, t_atom *argv) {
    if (argc && argv) {
        x->mode = CLAMP(atom_getlong(argv), 0, 2);
    }
    return 0;
}
...
t_max_err tide_attr_setsync(t_tide *x, void *attr, long argc, t_atom *argv) {
    if (argc && argv) {
        x->sync_enabled = atom_getlong(argv) ? 1 : 0;
    }
    return 0;
}
...
t_max_err tide_attr_setquantize(t_tide *x, void *attr, long argc, t_atom *argv) {
    if (argc && argv) {
        x->quantize_enabled = atom_getlong(argv) ? 1 : 0;
    }
    return 0;
}
...
t_max_err tide_attr_setslewrate(t_tide *x, void *attr, long argc, t_atom *argv) {
    if (argc && argv) {
        x->slew_rate = CLAMP(atom_getfloat(argv), 0.0, 1.0);
    }
    return 0;
}